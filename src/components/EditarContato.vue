<template>
  ...
  const formsDados = ref(props.dados);
  const formsEditado = ref(formsDados.value)
  const emit = defineEmits(["editForms"]);
  function editContatos() {
    //if(!formsDados.value.nome.trim() || !formsDados.value.telefone.trim() || !formsDados.value.email.trim() || !formsDados.value.cep.trim() || !formsDados.value.endereco.numero.trim() || formsDados.value == props.dados){
      //alert("Preencha todos os campos obrigatórios.");
      //return;
    //} else{
    emit("editForms", formsEditado.value);
  //}}
  }
